...
class LSTM(nn.Module):
    ...
    def forward(self, x, lengths):
        ...
        out_last = out[lengths-1, range(x.size(1))]
        # Decode the hidden state of the last time step
        out = self.fc(self.dropout(out_last))  # Only last time step is used for classification/regression
        return out
...
class build_rnn():
    ...
    def get_model(self, network_type = 'bilstm'):
        if network_type == 'bilstm':
            model = LSTM(self.input_size, self.n_hidden, self.n_layers, self.n_out, self.dropout).cpu()
        return model
